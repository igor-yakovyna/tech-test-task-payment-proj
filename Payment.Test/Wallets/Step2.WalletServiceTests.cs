...
public class Step3WalletServiceTests
{
    ...
    public void Test_ContributeToWalletDifferentCurrency()
    {
        ...
        Assert.Throws<ArgumentException>(() => sut.Contribute(createdWallet, contributor, amount),
    ...
    public void Test_Contribute_Should_Have_Correct_Shares_For_Non_Empty_Wallet()
    {
        ...
        Assert.AreEqual(Money.Euro(110), wallet.Shares.Single(sh => sh.Owner.Id == contributor1.Id).Amount,
            "contributor1 should have a share of 110");
        Assert.AreEqual(Money.Euro(50), wallet.Shares.Single(sh => sh.Owner.Id == contributor2.Id).Amount,
            "contributor2 should have a share of 50");
    }

    private static IWalletService GetWalletService()
    {
        IWalletRepository walletRepository = new WalletMemoryRepository();
